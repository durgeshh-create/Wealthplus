...
def send_daily_summary(backend: dict):
    ...
    try:
        lines = [
            f"📊 <b>WealthAlgo PS5673 — Today's P&L Summary</b>",
            f"📅 {_ist_now()}",
            f"{'─' * 30}",
        ]
...
def main():
    ...
    # ── Snapshot writer — writes /tmp/status_ps5673.json every 5 min ─────────
    ...
    try:
        ...
        # own try/except) could leave /tmp/status_ps5673.json never created.
        # The GitHub Actions pusher loop (`until [ -f status_ps5673.json ]`)
