...
def register_routes(app, dashboard_state):
    ...
    def intraday_start():
        ...
        ok = engine.start()
        return jsonify({'success': ok, 'running': engine.is_running})
    ...
    def sync_portfolio():
        ...
        try:
            ...
            portfolio.sync()
            logger.info("Portfolio synced manually via dashboard")
        ...
        try:
            ...
            def _compute(sym):
                ...
                if hist_df is not None and len(hist_df) >= period:
                    daily_wr = calculate_daily_williams_r(
                        hist_df,
                        live_price=ltp,
                        live_high=live_high if live_high and live_high >= (ltp or 0) else ltp,
                        live_low=live_low   if live_low  and live_low  <= (ltp or 0) else ltp,
                        period=period,
                    )
                ...
                return sym, daily_wr, weekly_wr, dma20, below_dma, ref_price
            ...
            with ThreadPoolExecutor(max_workers=20) as ex:
                ...
                for fut in as_completed(futures):
                    try:
                        sym, dwr, wwr, dma20, below_dma, ltp_price = fut.result()
                    except Exception:
                        sym = futures[fut]; dwr = wwr = None; dma20 = None; below_dma = False; ltp_price = None
