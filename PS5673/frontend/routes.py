...
def register_routes(app, dashboard_state):
    ...
    # ═══════════════════════════════════════════════════════════════
    # OPTIONS PAGE PROXY ENDPOINTS
    # These relay Kite API calls server-side so the static options.html
    # page can fetch live data without hitting Zerodha's CORS block.
    # ═══════════════════════════════════════════════════════════════

    @app.route('/api/options/quotes')
    def options_quotes():
        """
        Proxy: Nifty spot + India VIX live prices.
        Returns { nifty: float, vix: float, ts: str }
        Used by options.html to populate the range formula.
        """
        try:
            realtime = dashboard_state.get('realtime_manager')
            auth     = dashboard_state.get('auth_manager')

            if not realtime or not auth:
                return jsonify({'error': 'Bot not ready — log in first'}), 503

            nifty = realtime.get_ltp('NIFTY 50')
            vix   = realtime.get_ltp('INDIA VIX')

            # WebSocket may not have ticked yet — fall back to REST OMS quote
            FALLBACK_TOKENS = {'NIFTY 50': '256265', 'INDIA VIX': '264969'}
            for name, token in FALLBACK_TOKENS.items():
                val = realtime.get_ltp(name)
                if not val or val <= 0:
                    try:
                        qr = auth.session.get(
                            f"{Config.ZERODHA_API_BASE}/oms/quote",
                            params={'i': f'NSE:{token}'},
                            timeout=6,
                        )
                        if qr.status_code == 200:
                            qd = qr.json().get('data', {}).get(f'NSE:{token}', {})
                            price = qd.get('last_price')
                            if price:
                                if name == 'NIFTY 50':
                                    nifty = float(price)
                                else:
                        ...
                        logger.debug(f"options_quotes REST fallback for {name}: {fe}")

            if not nifty or not vix:
                return jsonify({'error': 'Could not fetch Nifty/VIX — market may be closed'}), 503
            ...
            return jsonify({'nifty': round(float(nifty), 2), 'vix': round(float(vix), 2), 'ts': ts})

        except Exception as e:
            logger.error(f"options_quotes error: {e}", exc_info=True)
            return jsonify({'error': str(e)}), 500
    ...
    def options_chain():
        """
        Proxy: Option chain for a given Nifty weekly expiry + strike.
        Query params:
          expiry  — DDMMMYY  e.g. 24JUN25
          strike  — integer  e.g. 24500
          type    — CE | PE
        Returns { ltp, delta, iv, oi } or { error }
        """
        try:
            auth = dashboard_state.get('auth_manager')
            if not auth:
            ...
            expiry = request.args.get('expiry', '').strip().upper()
            strike = request.args.get('strike', '').strip()
            opt_type = request.args.get('type', '').strip().upper()

            if not expiry or not strike or opt_type not in ('CE', 'PE'):
                return jsonify({'error': 'Required: expiry (DDMMMYY), strike (int), type (CE|PE)'}), 400

            url = f"{Config.KITE_API_BASE}/oi/chain/NIFTY"
            resp = auth.session.get(
                url,
            ...
            if resp.status_code != 200:
                # api.kite.trade/oi/chain needs X-Kite-Version header when using enctoken
                resp = auth.session.get(
                    url,
                    params={'expiry': expiry},
            ...
            if resp.status_code != 200:
                return jsonify({'error': f'Kite chain returned HTTP {resp.status_code}'}), resp.status_code

            chain = resp.json().get('data') or []
            entry = next((e for e in chain if str(e.get('strike_price')) == str(strike)), None)
            if not entry:
                return jsonify({'error': f'Strike {strike} not found in chain for expiry {expiry}'}), 404

            key = 'call_options' if opt_type == 'CE' else 'put_options'
            leg = entry.get(key) or {}
            oc  = leg.get('option_chain') or {}

            ltp   = leg.get('last_price') or oc.get('last_price')
            greeks = leg.get('greeks') or oc.get('greeks') or {}
            delta = greeks.get('delta') or oc.get('delta')
            iv    = greeks.get('iv')    or oc.get('iv')
            oi    = leg.get('oi')       or oc.get('oi')
            ...
            logger.error(f"options_chain error: {e}", exc_info=True)
            return jsonify({'error': str(e)}), 500
