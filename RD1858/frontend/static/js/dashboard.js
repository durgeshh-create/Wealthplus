...
function showBuyLiquidcaseModal() {
    ...
    fetch('/api/depth/LIQUIDCASE')
        .then(r => r.json())
        .then(d => {
            const bidQtyEl = document.getElementById('modal-liquidcase-bid-qty');
            const askQtyEl = document.getElementById('modal-liquidcase-ask-qty');
            ...
            // Top bid/ask quantities from depth array
            const topBidQty = d.buy_depth?.[0]?.quantity;
            const topAskQty = d.sell_depth?.[0]?.quantity;
            if (bidQtyEl) bidQtyEl.textContent = topBidQty != null ? `qty ${topBidQty.toLocaleString('en-IN')}` : 'qty —';
            if (askQtyEl) askQtyEl.textContent = topAskQty != null ? `qty ${topAskQty.toLocaleString('en-IN')}` : 'qty —';
...
function showSellLiquidcaseModal() {
    ...
    fetch('/api/depth/LIQUIDCASE')
        .then(r => r.json())
        .then(d => {
            const bidQtyEl = document.getElementById('sell-modal-liquidcase-bid-qty');
            const askQtyEl = document.getElementById('sell-modal-liquidcase-ask-qty');
...
function refreshFundsCards() {
    // Throttle to once per 5 min — cache/margin don't change second-to-second.
    // Forced refresh on: trade completion, portfolio tab open, manual refresh button.
    const now = Date.now();
    if (now - window._lastFundsFetch < 300000) return;   // 5 min
    window._lastFundsFetch = now;
...
function autoSyncPortfolio() {
    fetch('/api/portfolio/sync', { method: 'POST' })
        ...
        .then(data => {
            ...
            // If the sync detected a change (new buy/sell), force a fresh cash refresh
            if (data.changed || data.trades_detected) {
                window._lastFundsFetch = 0;
            }
            loadPortfolio();
            refreshFundsCards();
...
function forceBuyNow() {
    ...
    fetch('/api/force_buy_now', { method: 'POST' })
        ...
        .then(d => {
            ...
            if (d.success) {
                ...
                active.forEach(s => {
                    ...
                    const detail = (!s.success && s.reason) ? ` (${s.reason})` : '';
                    lines.push(icon + ' ' + s.symbol + detail);
                });
        ...
        .finally(() => {
            ...
            // Refresh cash/margin after buy — orders may have changed available funds
            setTimeout(() => { window._lastFundsFetch = 0; refreshFundsCards(); }, 3000);
        });
}
...
function forceSellNow() {
    ...
    fetch('/api/force_sell_now', { method: 'POST' })
        ...
        .finally(() => {
            ...
            // Refresh cash/margin after sell — funds released by closing positions
            setTimeout(() => { window._lastFundsFetch = 0; refreshFundsCards(); }, 3000);
        });
}
...
// ════════════════════════════════════════════════════════════════
// ════════════════════════════════════════════════════════════════
//  QUICK LOOKUP  (single-symbol instant scanner result)
// ════════════════════════════════════════════════════════════════

(function () {
    const LISTS = { etf: () => NSE_ETF_LIST, n200: () => NSE_NIFTY200_LIST, mid150: () => NSE_MIDCAP150_LIST };
    const CHUNK = 50;

    // ── FDD build / close ────────────────────────────────────────────────────
    window._buildScnLkFDD = function (listKey, inputEl) {
        const dd    = document.getElementById(`scn-lk-fdd-${listKey}`);
        if (!dd) return;
        const query = (inputEl.value || '').trim().toUpperCase();
        const pool  = LISTS[listKey]();
        ...
        if (!filtered.length) {
            dd.innerHTML = '<div class="scn-lookup-fdd-empty">No matches</div>';
            dd.style.display = 'block';
            return;
        }
        ...
        function renderItems(n) {
            const rows = filtered.slice(0, n).map(sym =>
                `<div class="scn-lookup-fdd-item" onmousedown="event.preventDefault();_scnLkPick('${sym}')">${sym}</div>`
            ).join('');
            const footer = n < filtered.length
                ? `<div class="scn-lookup-fdd-footer">${filtered.length - n} more — keep typing</div>` : '';
            dd.innerHTML = rows + footer;
        }
        renderItems(rendered);
        dd.style.display = 'block';
    ...
    window._closeScnLkFDD = function (listKey) {
        const dd = document.getElementById(`scn-lk-fdd-${listKey}`);
        if (dd) dd.style.display = 'none';
    };

    function _closeAllScnLkFDDs() {
        ['etf','n200','mid150'].forEach(k => {
            const dd = document.getElementById(`scn-lk-fdd-${k}`);
            if (dd) dd.style.display = 'none';
        });
    }

    document.addEventListener('mousedown', e => {
        if (!e.target.closest('#scn-lookup-row')) _closeAllScnLkFDDs();
    });

    // ── Symbol picked — clear other inputs, fire lookup ─────────────────────
    window._scnLkPick = function (sym) {
        _closeAllScnLkFDDs();
        // Fill picked input, clear the other two
        ['etf','n200','mid150'].forEach(k => {
            const inp = document.getElementById(`scn-lk-input-${k}`);
            if (!inp) return;
            const pool = LISTS[k]();
            inp.value = pool.includes(sym) ? sym : '';
        });
        _scnLookup(sym);
    };

    // ── Fetch single-symbol scan result ─────────────────────────────────────
    let _lkAbort = null;

    window._scnLookup = function (sym) {
        if (!sym) return;
        // Show spinner, hide card
        const spinner = document.getElementById('scn-lk-spinner');
        const card    = document.getElementById('scn-lk-card');
        if (spinner) spinner.classList.add('vis');
        if (card)    card.classList.remove('visible');

        // Cancel any in-flight request
        if (_lkAbort) { try { _lkAbort.abort(); } catch(_) {} }
        _lkAbort = new AbortController();
        ...
        .then(d => {
            ...
            // Backend drops symbols with no data into no_data array — handle both
            const r = results[0] || null;
            _renderLkCard(sym, r, d.no_data || [], dailyT, weeklyT);
        })
        .catch(err => {
            if (err.name === 'AbortError') return;
            if (spinner) spinner.classList.remove('vis');
            _renderLkCardError(sym, 'Network error');
        });
    };

    // ── Render the result card ───────────────────────────────────────────────
    function _pill(label, cls) {
        return `<span class="scn-lk-pill ${cls}">${label}</span>`;
    }

    function _fmtWR(val, thresh, label) {
        if (val == null) return _pill(`${label}: —`, 'neutral');
        const cls = val <= thresh ? 'good' : val <= -50 ? 'warn' : 'neutral';
        return _pill(`${label}: ${val.toFixed(1)}`, cls);
    }

    function _fmtAtv(val) {
        if (val == null) return '';
        if (val >= 1e7)  return _pill('₹' + (val/1e7).toFixed(2) + ' Cr', val >= 1e8 ? 'good' : 'warn');
        return _pill('₹' + (val/1e5).toFixed(2) + ' L', 'neutral');
    }

    function _fmtDma(r) {
        if (r.dma20 == null || r.ltp == null) return '';
        const pct    = ((r.ltp - r.dma20) / r.dma20) * 100;
        const pctStr = (pct >= 0 ? '+' : '') + pct.toFixed(2) + '%';
        const cls    = r.below_20dma ? 'bad' : 'neutral';
        return _pill(`vs 20DMA: ${pctStr}`, cls);
    }

    function _addBtns(sym, r) {
        if (!r) return '';
        const usedActive = (_symState && _symState.active) ? new Set(_symState.active) : new Set();
        const usedBnh    = (_symState && _symState.bnh)    ? new Set(_symState.bnh)    : new Set();
        const inActive   = usedActive.has(sym);
        const inBnh      = usedBnh.has(sym);
        const disActive  = inActive || inBnh;
        const disBnh     = inBnh    || inActive;
        return `<div class="scn-lk-actions">
            <button class="scn-lk-add-btn active"
                    ${disActive ? 'disabled title="' + (inActive ? 'Already in Active Strategy' : 'In Dip Accumulator') + '"' : ''}
                    onclick="_scnLkAddTo('${sym}','active')">➕ Active</button>
            <button class="scn-lk-add-btn bnh"
                    ${disBnh ? 'disabled title="' + (inBnh ? 'Already in Dip Accumulator' : 'In Active Strategy') + '"' : ''}
                    onclick="_scnLkAddTo('${sym}','bnh')">➕ Dip Acc.</button>
        </div>`;
    }

    function _renderLkCard(sym, r, noData, dailyT, weeklyT) {
        const card  = document.getElementById('scn-lk-card');
        const inner = document.getElementById('scn-lk-card-inner');
        ...
        if (!r) {
            // Symbol was in no_data — no local CSV and OMS fetch failed
            inner.innerHTML = `
                <span class="scn-lk-sym">${sym}</span>${badge}
                <span class="scn-lk-err">No data — seed required or market closed</span>`;
        } else {
            const ltpStr = r.ltp != null ? '₹' + r.ltp.toFixed(2) : '—';
            const sigCls = r.both_ok ? 'good' : r.daily_ok ? 'warn' : r.weekly_ok ? 'warn' : 'neutral';
            const sigLbl = r.both_ok ? '✅ Both signals' : r.daily_ok ? '📉 Daily only' : r.weekly_ok ? '📊 Weekly only' : '— No signal';
            inner.innerHTML = `
                <span class="scn-lk-sym">${sym}</span>${badge}
                <span class="scn-lk-ltp">${ltpStr}</span>
                <span class="scn-lk-sep">·</span>
                ${_fmtWR(r.daily_wr,  dailyT,  'D W%R')}
                ${_fmtWR(r.weekly_wr, weeklyT, 'W W%R')}
                ${_fmtDma(r)}
                ${_fmtAtv(r.avg_trade_val)}
                ${_pill(sigLbl, sigCls)}
                ${_addBtns(sym, r)}`;
        }
    ...
    function _renderLkCardError(sym, msg) {
        const card  = document.getElementById('scn-lk-card');
        const inner = document.getElementById('scn-lk-card-inner');
        if (!card || !inner) return;
        inner.innerHTML = `<span class="scn-lk-sym">${sym}</span> <span class="scn-lk-err">${msg}</span>`;
        card.classList.add('visible');
    }

    // ── Add selected symbol directly to a strategy ───────────────────────────
    window._scnLkAddTo = function (sym, strategy) {
        if (!_symState) return;
        const other      = strategy === 'active' ? _symState.bnh    : _symState.active;
        ...
        if (_symState[strategy].includes(sym)) { showToast(`${sym} already in ${thisLabel}`, 'warning'); return; }
        if (other.includes(sym)) { showToast(`⚠️ ${sym} is in ${otherLabel}`, 'error'); return; }
        _symState[strategy].push(sym);
        _renderSymList(strategy);
        _markUnsaved(strategy);
        showToast(`✅ ${sym} added to ${thisLabel} — remember to Save`, 'success');
        // Re-render card so buttons update to disabled
        const inner = document.getElementById('scn-lk-card-inner');
        if (inner) {
            const btns = inner.querySelectorAll('.scn-lk-add-btn');
            btns.forEach(b => { b.disabled = true; b.title = `Added to ${thisLabel}`; });
        }
    };
...
// Scanner sort state: 'dwr' | 'wwr' | 'dma' | 'atv'  ·  dir: 1=asc, -1=desc
let _scnSortCol = 'dwr';
...
function renderScanResults() {
    ...
    rows = rows.slice().sort((a, b) => {
        ...
        if (_scnSortCol === 'wwr') {
            ...
            av = ap; bv = bp;
        } else if (_scnSortCol === 'atv') {
            av = a.avg_trade_val != null ? a.avg_trade_val : (_scnSortDir > 0 ? -1 : Infinity);
            bv = b.avg_trade_val != null ? b.avg_trade_val : (_scnSortDir > 0 ? -1 : Infinity);
        } else { // 'dwr' default
    ...
    tbody.innerHTML = rows.map(r => {
        ...
        // Format avg trade value: ₹X.XX Cr (≥1Cr) or ₹X.XX L (lakh)
        let atvHtml = '—';
        if (r.avg_trade_val != null) {
            const v = r.avg_trade_val;
            let atvStr, atvClass;
            if (v >= 1e7) {
                atvStr  = '₹' + (v / 1e7).toFixed(2) + ' Cr';
                atvClass = v >= 1e8 ? 'scn-atv-high' : 'scn-atv-mid';
            } else {
                atvStr  = '₹' + (v / 1e5).toFixed(2) + ' L';
                atvClass = 'scn-atv-low';
            }
            atvHtml = `<span class="${atvClass}" title="Avg daily trade value (22d): ₹${Math.round(v).toLocaleString('en-IN')}">${atvStr}</span>`;
        }
        ...
        return `<tr class="${rowCls}" data-symbol="${r.symbol}">
            ...
            <td>${dmaHtml}</td>
            <td class="scn-atv-cell">${atvHtml}</td>
            <td>${sigHtml}</td>
            <td>${inStratHtml}</td>
        </tr>`;
    }).join('');
